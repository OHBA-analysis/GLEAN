...
for subspace = cellstr(settings.space)
    ...
    for session = 1:numel(GLEAN.data)
        ...
        if num_frequencies == 1
            session_maps(:,:,1,session) = glean_regress(GLEAN.(data).data{session},regressors(model.subIndx==session,:),res);
        else
            session_maps(:,:,:,session) = glean_regress(GLEAN.(data).data{session},regressors(model.subIndx==session,:),res);
        end
