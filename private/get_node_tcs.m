...
if ischar(voxelData) && strcmp(voxelData(end-3:end), '.mat') && exist(voxelData, 'file'), 
    ...
    voxelData = tmp.(ff{1});
    clear tmp;
    goodSamples = true(1,cols(voxelData));
...
switch lower(timeCourseGenMethod)
    case 'pca'
        ...
        assert(~any(row_sum(spatialBasis) > 1), ...
        ...
        for iParcel = nParcels:-1:1,
            ...
            if any(thisMask), % non-zero
                ...
                parcelData = demean(parcelData, 2);
                
                [U, S, V]  = fast_svds(parcelData, 1);
                PCAscores  = S * V';
    ...
    case 'peakvoxel'
        ...
        for iParcel = nParcels:-1:1,
            ...
            if any(thisMask), % non-zero
                ...
                nodeData(iParcel,:) = zeros(1, cols(voxelData));
            end%if
    ...
    case 'spatialbasis'
        ...
        scaledSpatialMaps = scale_cols(spatialBasis, ...
                                   mapSign ./                ...
        ...
        for iParcel = nParcels:-1:1, % allocate memory on the fly
            ...
            weightedTS  = scale_rows(weightedTS, thisMap(parcelMask));
            ...
            if any(thisMask), % the mask is non-zero
                ...
                nodeTS = zeros(1, cols(weightedTS));
