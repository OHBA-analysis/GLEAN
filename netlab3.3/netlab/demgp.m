...
randn('state', 42);
x = [0.15 0.2 0.25 0.3 0.5 0.55 0.7 0.75 0.8 0.85 0.9]';
x2 = [0.15 0.2 0.25 0.3  0.5 0.55  0.7 0.75 0.8 0.85 0.9; 0.1 0.15 0.2 0.25  0.45 0.5  0.65 0.7 0.75 0.8 0.85]';
ndata = length(x);
t = sin(2*pi*x(:,1)) + 0.5*exp(2*pi*x2(:,2)/2)+ 0.05*randn(ndata, 1);

xtest = linspace(0, 1, 50)';
xtest = [xtest(2:end)];
xtest = linspace(0, 1, 50)';
xtest2 = [xtest(2:end),xtest(1:end-1)];
...
flops(0)
net2 = gp(2, 'ratquad');
net2 = gpinit(net2, x2, t, prior);
flops(0)
[net2, options] = netopt(net2, options, x2, t, 'scg');
if flops_works
    rflops = flops;
end
...
plot(xtestplot, ytest, '-k');
plot(xtestplot, ytest+(2*sig), '-b', xtestplot, ytest-(2*sig), '-b');
%axis([0 1 -1.5 1.5]);
title('Squared exponential covariance function')
legend('data', 'function', 'GP', 'error bars');
hold off

cninv2 = inv(gpcovar(net2, x2));
[ytest2, sigsq2] = gpfwd(net2, xtest2, cninv2);
sig2 = sqrt(sigsq2);
...
ylabel('Target')
fplot('sin(2*pi*x)', [0 1], '--m');
plot(xtestplot2, ytest2, '-k');
plot(xtestplot2, ytest2+(2*sig2), '-b', xtestplot2, ytest2-(2*sig2), '-b');
%axis([0 1 -1.5 1.5]);
title('Squared exponential covariance function')
legend('data', 'function', 'GP', 'error bars');
hold off
